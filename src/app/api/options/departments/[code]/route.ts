...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    const option = await prisma.attendanceDepartmentOption.update({
      where: { code },
      data: {
        ...(parsed.data.label !== undefined
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
  try {
    ...
    const [studentCount, adminCount] = await Promise.all([
      prisma.student.count({ where: { department: code } }),
      prisma.admin.count({ where: { department: code } }),
    ]);
    ...
    await prisma.attendanceDepartmentOption.delete({ where: { code } });
