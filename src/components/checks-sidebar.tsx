...
export function ChecksSidebar({ username }: { username?: string }) {
  const pathname = usePathname();
  const router = useRouter();

  async function handleLogout() {
    await fetch("/api/checks/auth", { method: "DELETE" });
    router.push("/");
    router.refresh();
  }
...
export function ChecksShell({
  ...
  return (
    <SidebarProvider>
      <ChecksSidebar username={username} />
      <SidebarInset>
        <header className="flex h-14 shrink-0 items-center gap-2 border-b px-3 sm:px-4">
          ...
          <p className="min-w-0 flex-1 truncate text-xs text-muted-foreground sm:text-sm">
            <span className="hidden sm:inline">
              Department check reminders — inks and technical issues
            </span>
            <span className="sm:hidden">COC-ILIGAN Checks</span>
          </p>
          <PortalBackLink />
        </header>
